...
class Ui_MainWindow(object):
    ...
    def setupTitleArea(self):
        ...
        self.title_label = self.utilities.createLabel(
            text="Pixelizing",
    ...
    def setupNavbar(self):
        ...
        self.upload_button = self.utilities.createButton("Upload", self.button_style)
        self.reset_image_button = self.utilities.createButton("Reset", self.button_style)
        self.save_image_button = self.utilities.createButton("Save", self.button_style)

        self.quit_app_button = self.utilities.createButton("X", self.quit_button_style)
        self.utilities.adjust_quit_button(self.quit_app_button)
    ...
    def setupMainButtons(self):
        ...
        """
        self.show_noise_options_button = self.utilities.createButton(
            "Noise", self.button_style, self.show_noise_controls
        )
        self.show_filter_options_button = self.utilities.createButton("Filters", self.button_style, self.show_filter_controls)
        self.show_edge_detecting_options_button = self.utilities.createButton("Edge Detector", self.button_style, self.show_edge_detection_controls)
        self.show_metrics_button = self.utilities.createButton("View Metrics", self.button_style)
        self.show_refine_options_button = self.utilities.createButton("Refine Image", self.button_style, self.show_refine_image_controls)
        self.show_threshold_options_button = self.utilities.createButton("Thresholding", self.button_style)
        self.grayscaling_button = self.utilities.createButton("Gray Scaling", self.button_style)
        self.show_fourier_filter_options_button = self.utilities.createButton("Fourier Filters", self.button_style, self.show_fourier_filter_controls)
        self.upload_hybrid_image_button = self.utilities.createButton("Hybrid Image", self.button_style)
    ...
    def setupNoiseWidgets(self):
        ...
        back_button = self.utilities.createButton("Back", self.button_style, self.show_main_buttons)
        self.page_noise_layout.addWidget(back_button)

        # Noise Mode
        mode_title = self.utilities.createLabel("Current Mode", isHead=True)
        self.page_noise_layout.addWidget(mode_title)
        self.noise_mode_button = self.utilities.createButton("Uniform Noise", self.button_style, self.toggle_noise_mode)
        self.page_noise_layout.addWidget(self.noise_mode_button)

        # Uniform Noise
        uniform_title = self.utilities.createLabel("Uniform Noise", "Color:white;", isVisible=True, isHead=True)
        self.page_noise_layout.addWidget(uniform_title)

        uniform_label = self.utilities.createLabel("Noise Amount", "Color:white;", isVisible=True)
        self.page_noise_layout.addWidget(uniform_label)

        (self.uniform_noise_slider,
         uniform_slider_label,
         uniform_slider_layout) = self.utilities.createSlider(unit="%", style=self.slider_style, isVisible=True)
        self.page_noise_layout.addLayout(uniform_slider_layout)

        # Gaussian Noise
        gaussian_title = self.utilities.createLabel("Gaussian Noise", "Color:white;", isVisible=True, isHead=True)
        self.page_noise_layout.addWidget(gaussian_title)

        mean_label = self.utilities.createLabel("Mean", "Color:white;", isVisible=True)
        self.page_noise_layout.addWidget(mean_label)

        (self.mean_gaussian_noise_slider,
         mean_slider_label,
         mean_slider_layout) = self.utilities.createSlider(unit="%", style=self.slider_style, isVisible=True)
        self.page_noise_layout.addLayout(mean_slider_layout)

        # This second label was "Mean" in your original code
        stddev_label = self.utilities.createLabel("Standard Deviation", "Color:white;", isVisible=True)
        self.page_noise_layout.addWidget(stddev_label)

        (self.stddev_gaussian_noise_slider,
         stddev_val_label,
         stddev_layout) = self.utilities.createSlider(unit="%", style=self.slider_style, isVisible=True)
        self.page_noise_layout.addLayout(stddev_layout)

        # Salt & Pepper Noise
        salt_pepper_noise_title = self.utilities.createLabel("Salt & Pepper Noise", "Color:white;", isVisible=True, isHead=True)
        self.page_noise_layout.addWidget(salt_pepper_noise_title)

        salt_pepper_noise_label = self.utilities.createLabel("Noise Amount", "Color:white;", isVisible=True)
    ...
    def setupFilterWidgets(self):
        ...
        kernal_size_label = self.utilities.createLabel("Kernal Size", isHead=True)
        self.page_filter_layout.addWidget(kernal_size_label)
        self.filter_kernal_size_button = self.utilities.createButton(self.kernal_sizes_array[0], self.button_style, lambda: self.toggle_kernal_size(self.filter_kernal_size_button))
        self.page_filter_layout.addWidget(self.filter_kernal_size_button)

        average_filter_title = self.utilities.createLabel("Average Filter", "color:white;", isVisible=True, isHead=True)
        self.page_filter_layout.addWidget(average_filter_title)
        self.average_filter_button = self.utilities.createButton("Apply", self.button_style)
        self.page_filter_layout.addWidget(self.average_filter_button)

        gaussian_filter_title = self.utilities.createLabel("Gaussian Filter", "color:white;", isVisible=True, isHead=True)
        self.page_filter_layout.addWidget(gaussian_filter_title)
        gaussian_filter_sigma_label = self.utilities.createLabel("Sigma - σ", "Color:white;", isVisible=True)
        ...
        self.gaussian_filter_apply_button = self.utilities.createButton("Apply", self.button_style)
        self.page_filter_layout.addWidget(self.gaussian_filter_apply_button)

        median_filter_title = self.utilities.createLabel("Median Filter", "color:white;", isVisible=True, isHead=True)
        self.page_filter_layout.addWidget(median_filter_title)
        self.median_filter_button = self.utilities.createButton("Apply", self.button_style)
    ...
    def setupEdgeDetectionWidgets(self):
        ...
        # =========================================================================================================
        sobel_label = self.utilities.createLabel("Sobel Edge", isHead=True)
        self.page_edge_detection_layout.addWidget(sobel_label)
        self.sobel_edge_detection_button = self.utilities.createButton("Apply", self.button_style)
        self.page_edge_detection_layout.addWidget(self.sobel_edge_detection_button)

        # =========================================================================================================
        roberts_label = self.utilities.createLabel("Roberts Edge", isHead=True)
        self.page_edge_detection_layout.addWidget(roberts_label)
        self.roberts_edge_detection_button = self.utilities.createButton("Apply", self.button_style)
        self.page_edge_detection_layout.addWidget(self.roberts_edge_detection_button)

        # =========================================================================================================
        canny_label = self.utilities.createLabel("Canny Edge", isHead=True)
        self.page_edge_detection_layout.addWidget(canny_label)

        high_threshold_label = self.utilities.createLabel("High Threshold")
        ...
        (self.edge_detection_high_threshold_spinbox,
         ...
         edge_detection_high_threshold_layout) = self.utilities.createSpinBox(0, 100, 50)
        ...
        low_threshold_label = self.utilities.createLabel("Low Threshold")
        ...
        (self.edge_detection_low_threshold_spinbox,
         ...
         edge_detection_low_threshold_layout) = self.utilities.createSpinBox(0, 100, 50)
        ...
        self.canny_edge_detection_button = self.utilities.createButton("Apply", self.button_style)
        self.page_edge_detection_layout.addWidget(self.canny_edge_detection_button)

        # =========================================================================================================
        prewitt_label = self.utilities.createLabel("Prewitt Edge", isHead=True)
        self.page_edge_detection_layout.addWidget(prewitt_label)
        self.prewitt_edge_detection_button = self.utilities.createButton("Apply", self.button_style)
    ...
    def setupRefineImageWidgets(self):
        ...
        # =========================================================================================================
        normalize_label = self.utilities.createLabel("Normalize", isHead=True)
        self.page_refine_image_layout.addWidget(normalize_label)
        self.normalize_image_button = self.utilities.createButton("Apply", self.button_style)
        self.page_refine_image_layout.addWidget(self.normalize_image_button)

        # =========================================================================================================
        equalize_label = self.utilities.createLabel("Equalize", isHead=True)
        self.page_refine_image_layout.addWidget(equalize_label)
        self.equalize_image_button = self.utilities.createButton("Apply", self.button_style)
        self.page_refine_image_layout.addWidget(self.equalize_image_button)

        # =========================================================================================================
        label01 = self.utilities.createLabel("", isHead=True)
        self.page_refine_image_layout.addWidget(label01)
        label02 = self.utilities.createLabel("", isHead=True)
    ...
    def setupFourierFilterWidgets(self):
        ...
        # =========================================================================================================
        pass_filter_label = self.utilities.createLabel("Current Pass Filter", isHead=True)
        self.page_fourier_filter_layout.addWidget(pass_filter_label)
        self.pass_filter_button = self.utilities.createButton("High Pass Filter", self.button_style, self.toggle_pass_filter)
        self.page_fourier_filter_layout.addWidget(self.pass_filter_button)

        # =========================================================================================================
        raduis_control_title = self.utilities.createLabel("Raduis Control", isHead=True)
        self.page_fourier_filter_layout.addWidget(raduis_control_title)
        (self.raduis_control_slider,
         raduis_control_label,
         raduis_control_layout) = self.utilities.createSlider(0, 100, 50, "%", self.slider_style, )
        ...
        label03 = self.utilities.createLabel("", isHead=True)
    ...
    def setupImageGroupBoxes(self):
        ...
        self.original_groupBox, _ = self.utilities.createGroupBox(
            title="Original Image",
            size=QtCore.QSize(502, 526),
            style=self.groupbox_style,
            isGraph=False
        )
        self.processed_groupBox, _ = self.utilities.createGroupBox(
